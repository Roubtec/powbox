...
|---|---|---|---|
| **Plugin** | the 8 shared Claude skills | `Roubtec/agent-skills` | `dev-skills@roubtec` marketplace plugin — a cold first-boot install (fresh `claude-config` volume, Claude primary) runs synchronously so the skills are live the **first** session; every later start refreshes it in the background (applies next session) |
...
At container start the Claude entrypoint hook runs `/usr/local/bin/seed-claude-plugins.sh` (baked from `docker/shared/seed-claude-plugins.sh`; best-effort, bounded, never wedges start): it adds the `Roubtec/agent-skills` marketplace and installs the `dev-skills@roubtec` plugin the first time, then on every later start refreshes it to keep it current — running `claude plugin marketplace update roubtec` **and** the follow-up `claude plugin update dev-skills@roubtec` (a marketplace refresh alone only updates the catalog listing; the `plugin update` is what pulls the new skills into the installed cache). The agent-skills manifests carry no version field, so **every commit on `main` is a new SHA-version** — merging a change to `agent-skills` main *is* the release. The script decides sync-vs-async per plugin state so only the cold case ever blocks startup, and only briefly: on a **cold** (fresh) `claude-config` volume with Claude as the **primary** agent it installs in the **foreground**, bounded, before the entrypoint `exec`s claude, so the `/dev-skills:*` skills are live in the **first** session (Claude enumerates plugins once at startup; on timeout or a peer's lock it detaches the remainder to a background self-heal and start proceeds). A **warm** keep-current refresh is self-backgrounded, so it adds no start latency and its updates apply on the next session start (or after `/reload-plugins`). When Claude is the **non-primary** agent (`PRIMARY_AGENT=codex`) its hook runs before the firewall, so it skips the plugin; `entrypoint-core.sh` then converges it **after** `init-firewall.sh`, detached — so the network work is ordered behind the firewall and never delays the primary (Codex) prompt for skills that session never uses; those skills land in the next Claude session. A deliberately disabled plugin is respected and never re-enabled; an offline start logs a skip and self-heals on a later online start. Progress goes to `.powbox-plugin-bootstrap.log` on Claude's config volume (`$CLAUDE_CONFIG_DIR`, i.e. `~/.claude`).
