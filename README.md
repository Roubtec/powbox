...
|---|---|---|---|
| **Plugin** | the 8 shared Claude skills | `Roubtec/agent-skills` | `dev-skills@roubtec` marketplace plugin — installed/refreshed by a detached post-firewall bootstrap at every container start; a short bounded wait before the agent launches means a warm refresh usually applies **this** session (cold volume: first session needs `/reload-plugins` or a restart) |
