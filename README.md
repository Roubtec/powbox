...
Claude's statusline resolves the account by asking `claude auth status --json` rather than reading a cached name, so it reports whichever credential actually won. It shows the account's address when there is one, and otherwise names the source in play (`api key` for an `ANTHROPIC_API_KEY` session); it shows nothing when neither resolves, since an opaque key maps to no identity without a network round trip. The answer is cached per `session_id` with a 10-minute TTL, so the ~0.5s probe is paid once per session rather than on every render. That behavior reaches an existing container only after you delete `~/.claude/statusline-command.sh`, which is seeded no-clobber.
