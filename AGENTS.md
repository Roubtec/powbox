...
- Before the git/`safe.directory` steps, `entrypoint-core.sh` write-probes each dir-mounted `/workspace/<slug>` as `node` and, for any it cannot write, `sudo`s `fix-workspace-perms.sh` to `chown` it to `node`. This fixes native-Linux hosts that bind-mount a repo owned by a foreign uid (commonly root), where the agent otherwise gets EACCES on every `git`/edit. It is probe-gated, so it no-ops on Windows/WSL (FUSE bind mounts already honour node's writes) and on uid-matched hosts; self-hosted mode is exempt (its workspace volume is pre-seeded node-owned by the launcher). The chown is bounded with `find -xdev` so it never descends into the nested `node_modules`/`.worktrees` volume mounts.
