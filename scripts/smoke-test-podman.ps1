...
$script = @(
  ...
  'cleanup() { set +e; [ -n "$cid" ] && podman rm -f "$cid" >/dev/null 2>&1; podman network rm smoke-net >/dev/null 2>&1; if [ -n "$compose_proj" ]; then [ -n "$compose_dir" ] && docker compose -f "$compose_dir/docker-compose.yml" -p "$compose_proj" down -v >/dev/null 2>&1; for c in $(podman ps -aq --filter "label=com.docker.compose.project=$compose_proj" 2>/dev/null); do podman rm -f "$c" >/dev/null 2>&1; done; podman network rm "${compose_proj}_default" >/dev/null 2>&1; fi; [ -n "$compose_dir" ] && rm -rf "$compose_dir"; return 0; }'
  'trap cleanup EXIT'
  ...
  '# Up to THREE services: "hc" (/bin/true) MUST reach healthy; the negative-control'
  '# "bad" (/bin/false) MUST be driven to unhealthy; a shell-less "distroless" service'
  '# (appended below only when its image pulls -- see 4c) actively reproduces the Kalm2'
  '# break. No systemd here (cgroupfs), so the'
