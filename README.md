...
- `docker/base/Dockerfile`: shared toolchain image (Node.js, Python, PHP, Go, PostgreSQL 16, OPA, Git, shell utilities, and more) used by the unified agent image
...
- `docs/`: deep-dive design chapters (architecture, entrypoint & runtime, worktree `node_modules` hardlinks, skills refresh & provenance, rootless Podman) — referenced from the relevant README section and from [AGENTS.md](AGENTS.md), read on demand rather than front-loaded
...
|---|---|---|---|
| **Plugin** | the 8 shared skills to **both** agents — as the `dev-skills@roubtec` plugin for Claude, and (synced from the same marketplace clone) as Codex's copies | `Roubtec/agent-skills` | `dev-skills@roubtec` marketplace plugin — installed/refreshed by a detached post-firewall bootstrap at every agent-container start (only the short-lived image-store writer skips it); on primary-Claude launches a short bounded wait before the agent launches means a warm refresh usually applies **this** session (a Codex prompt never uses the plugin, so it does not wait — the refresh lands next session; cold volume: first session needs `/reload-plugins` or a restart). The same detached bootstrap **also syncs the Codex copies** of these 8 skills from the plugin's freshly-refreshed marketplace clone into the codex-config volume (a local, no-network sync ordered after the clone refresh; SHA-gated to a byte-for-byte no-op when unchanged), so Codex converges on the same `agent-skills` commit at the same container-recycle cadence instead of the image-bake cadence |
...
- **One workspace volume.** A per-instance `agent-ws-<container>` volume is mounted at `/workspace/<slug>` and holds the clone plus `node_modules`, `.worktrees`, and the pnpm store, Go caches, and ccache as ordinary subdirectories. The separate `agent-nm-*` / `agent-wt-*` shadow volumes are **not** created in this mode (there is no host filesystem underneath to shadow). Because the store, the worktrees, and the root `node_modules` now share **one** mount, `pnpm install` **hardlinks** everywhere — including the root `node_modules`, which falls back to copying in dir-mounted mode (separate mount → `link(2)` `EXDEV`).
...
The mechanical lifecycle is handled by image-baked helpers on `PATH` — `wt-bootstrap`, `wt-enter`, `wt-remove` (plus `gitcat` for cross-branch reads and `gh-review-threads` for a concurrency-safe, PR-scope-checked review-thread fetch) — which the worktree skills and workflows call rather than re-deriving git plumbing. Orphan reaping never deletes work: a dir that is no longer a live worktree is removed only when empty, and otherwise preserved under `.worktrees/.orphaned/`. The measured copy→hardlink rationale, the durable-metadata/orphan-handling model, and tmpfs sizing are detailed in [docs/worktree-node-modules-hardlinks.md](docs/worktree-node-modules-hardlinks.md).
...
This runs a hermetic **Stage 0** up front — the `sensitive-host-path` predicate + `/proc/self/mountinfo` parser unit test (`scripts/test-sensitive-host-path.sh`, needing no image, root, or native-Linux host), which guards the classifier that stops the workspace-perms heal from recursively chowning a mount whose host source is a system/home directory — then six image/host stages: a fast presence sweep over every expected CLI; a `pg-dev-up` functional test that stands up a throwaway PostgreSQL cluster and connects through the emitted `DATABASE_URL` (exercising role/db creation, URL encoding, and host binding — things the presence check alone can't); a rootless-Podman engine test that runs the image with the launch-time device + security wiring and exercises a nested container run, a bridge published port, and the `podman compose` subcommand (so a base/Podman bump that regresses the engine is caught — see [docs/rootless-podman.md](docs/rootless-podman.md)); and a self-hosted (`--isolated`) launch test that checks the launcher's identity contract (no image, daemon, or network needed) and — only when the agent image is present — the baked `seed-workspace.sh` clone/reuse/`--reclone`/failure and single-mount hardlink behavior (it self-skips that image-dependent half when the image is absent); and a native-Linux dir-mount ownership test that bind-mounts root-owned and mixed-ownership (node-owned root with nested root-owned files) git fixtures and asserts the `node` agent can write and `git commit` each after the tree is re-owned — the all-root case drives the extracted entrypoint decision unit `heal-workspace-perms.sh` (the byte-for-byte probe-and-call logic `entrypoint-core.sh` runs, so the stage guards that decision path, not only the baked `fix-workspace-perms.sh` helper + sudoers wiring it ultimately invokes), and the mixed-ownership case drives that same unit so the stage also guards the entrypoint's nested-uid-0 scan (PR #55 plus task 005a for the all-root real-entrypoint drive; task 007 for the mixed case, and task 007a to drive it through the same unit), driving the unit hermetically rather than booting the full entrypoint chain, self-skipping unless it is on a native-Linux host where a root-owned fixture can be created (the local-dev case self-skips; CI runs it for real) and the image is present; and a durable worktree-metadata recreate test (task 017) that launches two throwaway containers on one named `agent-wt`-style volume — the first binds the volume's git metadata over `.git/worktrees` via the real `shadow-mounts.sh` and leaves a linked worktree dirty (a tracked modification plus an untracked file), the second recreates on the SAME volume and asserts `git status` still works, the branch/HEAD and both dirty changes survived, and the dir-mounted checkout's own `.git/worktrees` gained no container registrations — so a broken bind or a metadata path that does not resolve after recreate is caught (it needs the image and a runtime that can grant `CAP_SYS_ADMIN` for the `mount --bind`, self-skipping otherwise; CI runs it for real). On a host that cannot expose `/dev/net/tun` (e.g. the Docker Desktop VM under the default `auto`), the Podman stage still validates the static engine wiring (engine present, the `containers.conf` drop-in, `podman info`, the `compose` subcommand) but skips only the nested-run/published-port checks; force the full check with `POWBOX_PODMAN=on`. A genuinely broken image (missing engine, dropped drop-in) fails the stage on any host. Skip the DB stage with `POWBOX_SMOKE_SKIP_DB=1`, the Podman stage with `POWBOX_SMOKE_SKIP_PODMAN=1`, the self-hosted stage with `POWBOX_SMOKE_SKIP_SELFHOSTED=1`, the dir-mount stage with `POWBOX_SMOKE_SKIP_DIRMOUNT=1`, and the worktree durable-metadata stage with `POWBOX_SMOKE_SKIP_WORKTREE_META=1` — these are independent, so set all five for a Stage 1 tools-only presence sweep that touches no network or container (PowerShell: `.\commands\smoke-test.ps1 -SkipDb -SkipPodman -SkipSelfHosted -SkipDirMount -SkipWorktreeMeta`).
...
The container also seeds a higher multi-agent concurrency default: a missing `[features]` block gains `multi_agent_v2 = true`, enabling Codex's second-generation multi-agent subsystem whose built-in concurrency (≥8 threads) roughly doubles the legacy `[agents]` ceiling (~4, one reserved for the root) — so powbox's parallel fan-outs (`address-tasks`/`address-reviews` and Codex-side delegation) get a wider ceiling out of the box.
This seed is no-clobber: if you already set `multi_agent_v2` (to `true` or `false`) it is left untouched, and because `multi_agent_v2` is still a pre-release feature the seed deliberately does **not** add `suppress_unstable_features_warning`, so Codex's unstable-feature notice stays visible as an honest reminder that v2 is opt-in.
**Migration caveat:** Codex refuses to launch when a config sets **both** an `[agents]` block and `features.multi_agent_v2` — they are mutually exclusive. To avoid authoring that conflict, the seed is skipped entirely whenever your config already carries an `[agents]` block (table, subtable, or inline), so v2 is never enabled behind an existing `[agents]` setting. If you want v2's higher concurrency on such a config, delete your `[agents]` settings (e.g. `max_threads`/`max_depth`) yourself — the no-clobber seed will not remove them for you, and leaving both in place keeps Codex on the old ceiling.
