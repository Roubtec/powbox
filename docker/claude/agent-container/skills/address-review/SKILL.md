...
- **`ping-contributing` prunes the ping set to the bots still adding value.** Without it, every named `ping-*` bot is re-pinged on each advancing push. With it, a named bot is re-pinged only when it *brought a new finding* this round (see its table row for what counts). Decide this per comment author from this round's triage. Supplied **alone** (no explicit bot named), the candidate set is every known bot (codex/claude/copilot) that authored a review thread this round; supplied alongside explicit names, it intersects with them. A round in which no candidate bot brought a new finding pings no bot — which, like the no-op-push skip, lets an automated multi-bot loop wind down reviewer-by-reviewer as each one goes quiet, even while the push itself advanced (e.g. you fixed a human's thread). It never *adds* a bot you did not name; it only ever removes one that stopped contributing.
