...
|---|---|---|---|
| **Plugin** | the 8 shared Claude skills | `Roubtec/agent-skills` | `dev-skills@roubtec` marketplace plugin, kept current at session start |
...
At container start the Claude entrypoint hook runs `docker/shared/seed-claude-plugins.sh` (fully detached, best-effort, never blocking start): it adds the `Roubtec/agent-skills` marketplace and installs the `dev-skills@roubtec` plugin the first time, then on every later start refreshes the marketplace to keep it current. The agent-skills manifests carry no version field, so **every commit on `main` is a new SHA-version** — merging a change to `agent-skills` main *is* the release, and the next session start picks it up (plugin updates apply on restart). A deliberately disabled plugin is respected and never re-enabled; an offline start logs a skip and self-heals on a later online start. Progress goes to `$CLAUDE_CONFIG_DIR/.powbox-plugin-bootstrap.log`.
