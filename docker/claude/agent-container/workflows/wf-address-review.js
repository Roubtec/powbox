...
- UNRESOLVED review threads via GraphQL \`reviewThreads\` — as SINGLE-SHOT queries, never \`gh api graphql --paginate\` (run concurrently with other gh GraphQL calls it has returned ANOTHER PR's threads); include \`totalCount\` + \`pageInfo{ hasNextPage endCursor }\` and page past 100 threads by passing the returned cursor to a fresh call. SCOPE-CHECK the result: every comment \`url\` must contain \`/pull/<this PR's number>\` — on any mismatch, discard the entire response and re-fetch; never emit an item whose \`url\` points at a different PR. Keep only \`isResolved == false\`. Emit each as \`type: "review-thread"\` with \`threadId\` (node id), \`commentId\` (top comment databaseId), \`path\`, \`line\`, \`author\` (login), \`authorIsBot\` (true when the comment's GraphQL \`author.__typename\` is \`Bot\` — query \`author{ login __typename }\`; do not guess from the login), \`body\`, \`url\`. \`threadId\` and \`commentId\` are mandatory for these — they are how publication resolves and replies.
