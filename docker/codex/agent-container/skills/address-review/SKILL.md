...
**Arguments:** `[PR#] [rebase on top of <branch>] [push] [hands-off] [ping-codex] [ping-claude] [ping-copilot] [ping-contributing]`
...
| `ping-contributing` | A **modifier** on the ping set: re-ping a bot only if it **brought a new finding this round.** Combined with explicit `ping-codex`/`ping-claude`/`ping-copilot`, it filters that named set down to the contributors; supplied **alone**, it falls back to every known bot (codex/claude/copilot) that reviewed this round. **Implies `push`** and is still subject to the no-op-push skip. The point: keep one fixed reviewer set across rounds and let a bot that has gone quiet drop out of the ping cycle on its own — so a multi-bot review→address loop winds down bot-by-bot instead of pinging everyone forever. *Brought a new finding* = authored ≥1 thread this round that surfaces a real concern not raised before on this PR (typically `actionable-fixed`, or a genuinely new `deferred-to-task`/`already-addressed`); it does **not** count a `push-back` (the comment was wrong), a re-raise of a concern already deferred to a committed task, or a bot re-arguing a push-back it already lost — **unless** that thread carries a genuinely new angle this round. |

### Flag interactions

- **`ping-*` implies `push`.** If `ping-codex`, `ping-claude`, `ping-copilot`, or `ping-contributing` is present without `push`, push anyway — a re-review of unpushed work is meaningless.
...
- **`ping-contributing` prunes the ping set to the bots still adding value.** Without it, every named `ping-*` bot is re-pinged on each advancing push. With it, a named bot is re-pinged only when it *brought a new finding* this round (see its table row for what counts). Decide this per comment author from this round's triage. Supplied **alone** (no explicit bot named), the candidate set is every known bot (codex/claude/copilot) that authored a review thread this round; supplied alongside explicit names, it intersects with them and never adds a bot you did not name. A round in which no candidate bot brought a new finding pings no bot — which, like the no-op-push skip, lets an automated multi-bot loop wind down reviewer-by-reviewer as each one goes quiet, even while the push itself advanced (e.g. you fixed a human's thread).
