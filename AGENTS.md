...
- Ephemeral agent-settings overlay: `docker/shared/shadow-agent-config.sh` (sudo-scoped; bind-mounts a `/dev/shm` copy over `settings.json` / `config.toml` when `AGENT_SETTINGS_EPHEMERAL=1`)
...
- `/etc/sudoers.d/node` must stay scoped to `/usr/local/bin/init-firewall.sh`, `/usr/local/bin/shadow-mounts.sh`, `/usr/local/bin/shadow-agent-config.sh`, and `/usr/bin/apt-get` only (mode `0440`).
