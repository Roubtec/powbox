...
Per-write atomicity is not atomicity across the **pair**, though, and a digest-carrying marker is where that bites: the asset and its marker are two independent renames, so two containers publishing concurrently can interleave them and leave one's asset under the other's `sha256=`, after which the digest never matches again and the asset reads as customized until it is deleted.
The statusline accepts that race for the reason [entrypoint-and-runtime.md](entrypoint-and-runtime.md) gives (an flock there would sit on the synchronous startup path); serializing the transaction is task 002h, and a consumer that is not on that path should reach for the cross-container lock `seed-claude-plugins.sh` already uses.
