...
- **ANTHROPIC_API_KEY in `.env`**: Set it if OAuth sessions prove unreliable in the container. `compose.agent.yml` already passes the variable through, so this is only about where the value comes from — and note it is not a *fallback* that waits for OAuth to fail: Claude Code checks the env credential ahead of the stored login (see README "Runtime", the paragraph on both API keys), so headless `claude -p` switches credential outright and an interactive session switches once you approve the detected key.
