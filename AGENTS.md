...
- Image-baked skills are seeded into each agent's user-level skills directory at startup (`$CLAUDE_CONFIG_DIR/skills/` for Claude, `~/.agents/skills/` for Codex), no-clobber at the skill-directory level; a per-repo `.claude/skills/` (or `.agents/skills/`) still takes precedence at invoke time. Because of the no-clobber, a rebuilt image's updated skill text does not replace the stale volume copies; `commands/update-skills.*` (function `agent-update-skills`) force-refreshes the baked skills onto both config volumes via a throwaway container, running the shared in-container worker `commands/update-skills-incontainer.sh` (whose SKILLS_SRC/SKILLS_DEST pairs mirror the entrypoint hooks). User-authored skills not baked into the image are left untouched.
