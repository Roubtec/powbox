...
Codex requires `OPENAI_API_KEY` set on the host before launching (interactively or headless); Claude optionally accepts `ANTHROPIC_API_KEY`. The key is not a *standby* credential waiting for the OAuth session to lapse: Claude Code checks `ANTHROPIC_AUTH_TOKEN`, `ANTHROPIC_API_KEY` and `CLAUDE_CODE_OAUTH_TOKEN` ahead of the stored login with no fall-through, so headless `claude -p` uses a key that is merely present — which is exactly why `peer-review-run` clears all three with `env -u` when it wants the subscription session. An interactive session additionally asks you to approve a detected key, so there it wins only once approved.
Because a statusline cannot observe that approval, Claude's shows `env auth` in place of the account name whenever one of the three variables is set: a signal that the cached OAuth name may not be the identity in use, not a claim about which credential won. That label reaches an existing container only after you delete `~/.claude/statusline-command.sh`, which is seeded no-clobber.
...
**Credential policy for a delegated peer:** a logged-in subscription session is primary and an inherited env credential is the fallback — the reverse of the raw precedence above, and powbox's own policy rather than Claude Code's behavior. `peer-review-run` is what enforces it, attempting the login first (with `env -u` clearing all three env credentials) and retrying in key mode only when that attempt yields no verdict. `docker/shared/peer-review-run` and [docs/architecture.md](docs/architecture.md) cite this paragraph as the policy's home.
