...
- **Stage 1 — tool presence and key image config** (`scripts/smoke-test-image.sh`). The sweep every later stage assumes: expected CLIs resolve (most are genuinely invoked, a handful are presence-only probes), plus the image configuration that would otherwise regress silently. Watch `powbox-provenance` and `gitcat` in particular: no suite anywhere in the smoke exercises them, so their presence probe here is the whole of their coverage. `wt-bootstrap` is the partial case, and less covered than it first looks: the binary itself is never invoked either, and what Stages 0c/0d exercise directly is the reaping primitive it delegates to — `wt-common.sh`'s `wt_reap_orphan_dir` — not `wt-bootstrap`. Everything the script does around that one call rests on `command -v wt-bootstrap` alone: main-repo-root resolution, the container-local mountpoint checks, the live-vs-orphan classification loop, the per-slug golangci-cache cleanup, the reap-outcome handling, the remote push probe, the headroom measurement, and the single-JSON-object output contract. One structural note that reading the probe list will not tell you: each probe is handed to the driver as a **separate argument** and executed in its own `sh -ec`, so probe text is data rather than script and a failure is reported by index against a printed manifest — but all probes share one container, so filesystem effects deliberately carry forward between them while `cd`, `export` and shell variables do not.
