...
- (Added in review) A cross-PR url arriving on a nested COMMENTS page, not a threads page: every other case feeds the scope check only threads-page urls, so a helper validating those before merging the fetched-up comments would pass. Nested pages carry no identity fields, so this scope check is the only guard between them and stdout. Also mutation-verified. The malformed-shape half of that path still fails open in the helper itself — see `tasks/033a-gh-review-threads-nested-comments-page-fails-open.md`.
