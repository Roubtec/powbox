...
PowBox handles this at startup via a narrowly-scoped, sudo-allowlisted root helper (`docker/shared/fix-workspace-perms.sh`), gated on a real write-probe **plus** a short-circuiting scan for nested `root`-owned (uid 0) files — so it stays a no-op whenever `node` can already write the whole tree (all Windows/WSL/macOS launches, and Linux hosts whose mount uid already matches `node` with no nested uid-0 entries):
...
- **Mixed ownership — node-owned root with nested `root`-owned (uid 0) files** — arises when a host operation runs as **root** against the live bind mount while the container exists, most often `sudo git pull` on a native-Linux host: it re-owns to uid 0 only the paths it writes (new `.git/objects/*`, refs, and the changed working-tree files), leaving the top directory `node`-owned. The root write-probe passes, yet `git commit`/`add` still fail with `insufficient permission for adding an object to repository database`. A short-circuiting nested-uid-0 scan in the entrypoint catches this case the probe alone misses and hands it to the same helper, which re-owns **only** those uid-0 entries to `node:node` — the already-node-owned root and any file owned by another host uid are untouched — so the agent self-heals on container start instead of needing a manual host-side `chown -R`.
...
This native-Linux bind-mount uid expectation — a `root`-owned (or mixed-ownership) mount must end up writable by `node` (uid 1000) — is regression-guarded by the dir-mount stage of the smoke test (`scripts/smoke-test-dirmount.{sh,ps1}`, Stage 5 of `commands/smoke-test.*`). It builds two git fixtures — a genuinely `root`-owned tree and a mixed-ownership one (a `node`-owned root hiding a nested `root`-owned tracked file and a `.git/objects/<xx>` shard) — and asserts `node` can edit and `git commit` each once the tree is re-owned. The all-root case drives the **genuine extracted entrypoint decision unit** (`heal-workspace-perms.sh`, the byte-for-byte code `entrypoint-core.sh` runs), invoked as `node`: its own write-probe and nested-uid-0 scan decide *whether* and *with what path/sudo* to call `fix-workspace-perms.sh`, so it guards that probe-and-call **decision** path — not only the baked helper plus its sudoers wiring, which the unit still exercises by the same allowlisted path (that coverage is subsumed, not lost). The stage drives the unit hermetically and still does **not** boot the full entrypoint chain — the firewall/gh/shadow setup needs the launcher's compose wiring and is out of scope. The mixed-ownership case drives that same unit, so the stage also guards the entrypoint's nested-uid-0 scan — the trigger that catches a `node`-owned root hiding nested uid-0 files, which the root-level write probe alone misses — and reverting that scan leaves the mixed fixture unhealed and fails the stage (task 007a). Because the bug is native-Linux-only (Windows/WSL/macOS bind mounts mask it) and a `root`-owned fixture needs root, the stage self-skips off native-Linux and on hosts without root/passwordless sudo, and runs for real on the CI runner.
...
**Baked helpers.** The mechanical worktree lifecycle is encoded once as three image-baked scripts on `PATH`, shared by both agents' worktree-running skills and the Claude dynamic workflows: `wt-bootstrap` (verify the roots are container-local, prune this container's orphans, probe remote access; prints JSON), `wt-enter <slug> <branch> [<base>]` (rerun-safe resolve/attach/create of a worktree under `.worktrees/$CONTAINER_NAME/`; prints the path), and `wt-remove <slug> [--force]` (guarded removal that refuses to delete uncommitted work and never deletes branches). Skills and workflow prompts invoke these instead of re-deriving the git plumbing in prose. A fourth baked git helper, `gitcat [--raw] <ref> <path> [<start> [<end>]]`, prints a file as of another branch or commit with stable line numbers — it makes cross-branch reads (e.g. reviewing a stacked-PR chain from one checkout) a single command instead of a `git show`-to-`/tmp` round-trip.
