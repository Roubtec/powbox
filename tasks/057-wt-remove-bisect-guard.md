...
**Prerequisite: land PR #125 (task 039, branch `task/039-wt-enter-primary-checkout`) first.** That PR is what adds the `scripts/test-wt-orphan-safety.sh` fixtures this task builds on — interrupted rebase, bisect (detached and on-branch), interrupted revert — and the `destructive_advice` detector the acceptance criteria below name. None of them exist on `main` today; they live only on #125's branch, which is still open. Schedule this task after #125 is merged, or at minimum branch this work off #125's head and note the dependency in the PR — proceeding without either is possible but costs strictly more, because the fixtures and the detector then have to be written from scratch here. That cost is the reason to wait; it is not licence to drop an acceptance criterion, all of which remain satisfiable either way.
