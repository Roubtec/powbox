...
Half (1) is automated for **both** drivers today: Tier 1 runs the Bash umbrella's Stage 6 and a dedicated PowerShell Stage 6 step, on a runner where the assertions have real teeth (native Linux, rootful daemon, unprivileged `runner` invoker, uid 1001).
Task 053a (PR #152, merged) is what added the PowerShell driver's own mountpoint-ownership assertions and that dedicated Tier 1 step; before it, `scripts/smoke-test-worktree-metadata.ps1` carried a KNOWN DIVERGENCE comment and asserted none of them.
...
- **Derived image.** `FROM powbox-agent:latest` plus a `RUN` that strips the `chown` from `/usr/local/bin/shadow-mounts.sh`, then point the driver at the derived tag via its existing `-Image` / `$IMAGE` argument. No driver change at all — the drivers already take the image as a parameter. Costs one extra tiny layer build. Verified feasible: the script is baked by a plain `COPY --chmod=755` as root (`docker/base/Dockerfile`) with **no** `chattr +i` anywhere in `docker/`, so a root `RUN` can patch it; the "immutable" wording in the Dockerfile comments means root-owned relative to the `node` user, not filesystem-immutable.
  The derived Dockerfile must select `USER root` for that `RUN` and restore `USER node` after it: both stages end on `USER node` (`docker/base/Dockerfile`, `docker/agent/Dockerfile`), a derived stage inherits that final user, and the target is a root-owned mode-755 file under a root-owned directory — so an inherited-user `sed -i` cannot write it or its rename temp and the build fails before the control ever runs. Restore the user for the same reason the route was chosen: the drivers must exercise the mutated image exactly as they exercise the real one, so the mutation has to be the only difference between the two.
...
Settle this together with (c): which driver runs decides nothing on its own, because the mutation chosen there decides whether any driver's *own* assertions are reached at all. Only (c)'s **narrowed** mutation reaches them; under **match either message** the run aborts in the shared Container A payload first, so `owner_of` / `Get-PathOwnerId` never execute and no choice made here can change that.

- Tier 1, after the two Stage 6 steps, against **one** driver only — cheapest. Paired with the narrowed mutation it catches the "assertions can no longer fail" regression *in the driver it runs*; paired with **match either message** it catches that regression in **neither** driver, the one it runs included, and proves only the shared in-container check — so the `Get-PathOwnerId` / `owner_of` returning one sentinel for both paths case named above stays invisible in the covered driver too, not just in the other one. Even at its best — narrowed mutation, one driver — be explicit about what it still does not catch: 053a deliberately duplicates the host-side assertion code rather than sharing it, so a mutation-blindness confined to the other driver stays invisible, because the covered driver still goes red and the control still passes. Choosing this option therefore also means narrowing this task's claim to the one driver's assertions, rather than to the ownership coverage as a whole.
- Tier 1 against **both** drivers — twice the cost, and the only option that *can* cover both host-side assertion implementations. It buys that only alongside the narrowed mutation, which is what reaches them; alongside **match either message** both runs abort in the same shared payload and the second run proves nothing the first did not. Under the narrowed mutation, what it buys over the option above is precisely the per-driver blindness that option cannot see, which is the threat this task was written for. Since 053a merged there are two host-side implementations in the tree, so both runs have real assertions to exercise.
...
Removing the `chown` aborts the driver inside **Container A**, before any host-side assertion runs: the shared Container A script creates `$WS/.claude`, compares that intermediate directory against its pre-existing parent right there in the container, and exits with `the shadow-mounts.sh mountpoint chown REGRESSED` (`scripts/smoke-test-worktree-metadata-container-a.bash:148` as of this writing), which each driver reports as `Container A could not set up the durable worktree`.
The host-side message `the created mountpoint did NOT inherit the tree's ownership` is therefore unreachable under that mutation — in **both** drivers alike, since 053a gave them the same Container A script — and a control anchored on it alone would reject the very mutation it exists to prove.

That is a decision, not a detail — pick one:

- **Match either message**, accepting the in-container intermediate-directory check as a valid anchor, and record that the mutation is caught at the earliest ownership check rather than at the host-side ones. Cheapest, but it leaves the host-side assertions themselves unproven by this control — in **every** driver the control runs, since the abort is in the payload they share, which is why (b) cannot narrow that gap to one driver.
...
Whichever is chosen, note also that the host-side message now lives in both drivers — `scripts/smoke-test-worktree-metadata.sh` and, since 053a, `scripts/smoke-test-worktree-metadata.ps1` — but with each file's own punctuation (an em dash in the `.sh`, a hyphen in the `.ps1`), so a literal anchor must tolerate both, and the anchor set must be written against whichever drivers the control actually runs — see (b).
...
- The control's stated reach matches decisions (b) and (c) **together**: a single-driver control does not claim to protect both drivers' duplicated host-side assertion implementations, and a control whose mutation aborts in the shared Container A payload claims no host-side reach at all — not even in the driver it runs.
