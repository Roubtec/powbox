...
Why this is safe and durable: the common `.git` (objects + refs) is *not* shadowed, so committed work persists on the host and survives container recycle; only the ephemeral worktree scaffolding lives in tmpfs. powbox's README "Workspace Shadow Mounts → Git Worktree Parallel Development" has the full model (readable at `/ctx` if the powbox repo happens to be mounted, but this skill ships the contract so that is optional).
