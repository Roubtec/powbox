...
- Enforce and verify mode 0600 after writing the normalized file and before emitting a `passed` or `issues` result, using the same fail-closed standard as `ensure_private_dir`: if `chmod` fails, the mode cannot be read, or the filesystem does not honor 0600, emit no successful peer result and exit non-zero with a clear helper operational error. The enclosing verified-0700 directory does not make file-mode enforcement best-effort.
- Check the write itself, not only the mode. `peer-review-run` deliberately runs without `set -e`, so a redirection that fails part-way — a full or read-only artifact filesystem, a quota hit mid-write — leaves an empty or truncated file behind while the subsequent `chmod 0600` and mode read both succeed on it. A mode-only check therefore emits `passed` or `issues` with a `reviewFile` that silently violates the full-prose contract, which is worse than an outright failure because the caller relays the truncation as the peer's review. Status-check the normalized-file write and fail closed on a failed or short write exactly as on a failed chmod.
...
- Normalize the already-parsed final provider message into a new file inside the final attempt's returned `artifactDir`, checking that write's exit status, then chmod and verify exact mode 0600 before constructing a `passed` or `issues` terminal result; fail the helper operationally if either the write or the mode enforcement cannot be proved. Without `set -e` the write's status has to be tested explicitly at the redirection, since a later successful `chmod` says nothing about whether the bytes landed. Retain earlier attempts as sibling directories under the existing session parent, and do not make the stable payload path an alias for undocumented provider-native files.
...
- A test worktree carrying shadow `tomllib.py` and `sitecustomize.py` modules, with the helper invoked from that worktree as its working directory, resolves the real standard-library parser and executes neither shadow; configured-model resolution behaves exactly as it does from a clean directory, and the case fails loudly if the isolation flag is removed.
...
- A fixture filesystem or shim that rejects `chmod 0600`, makes mode unreadable, or silently ignores the change proves the review-file path fails closed: the helper exits non-zero with an operational diagnostic and never emits a `passed` or `issues` result claiming a usable `reviewFile`.
- A fixture that fails the normalized-file write while leaving `chmod` and the mode read succeeding — a full or read-only artifact filesystem, or a shim that truncates — proves the same fail-closed behavior, so an empty or partial `reviewFile` can never ship inside a successful result.
...
- Run the configured-model cases from a working directory that contains shadow `tomllib.py` and `sitecustomize.py` fixtures and assert neither executes, so parser isolation is proved rather than assumed.
