...
| `/workspace/<project-slug>/.worktrees` | Per-container worktrees volume (`agent-wt-<agent>-<project>`); also holds durable per-worktree git metadata at `.worktrees/.gitworktrees` (bind-mounted over `.git/worktrees`), the per-container pnpm store at `.worktrees/.pnpm-store` (JS/powbox gate only), Go caches (`.worktrees/.gomodcache`, `.worktrees/.gocache`, per-worktree `.worktrees/.golangci-cache/…`), the shared NuGet packages folder (`.worktrees/.nuget`), the shared Playwright browsers cache (`.worktrees/.ms-playwright`), and opt-in ccache (`.worktrees/.ccache`); dir-mounted JS/powbox, `go.mod`, or boundedly detected .NET projects |
| `/workspace/<repo-slug>[-<name-slug>]-<instance-hash>` | Self-hosted (`--isolated`) per-instance workspace volume (`agent-ws-<container>`) — the clone plus `node_modules`, `.worktrees`, and the pnpm store / Go caches / NuGet packages / Playwright browsers / ccache as subdirs; replaces the bind mount and the two volumes above |
...
- Static lint gates: `shellcheck` (Tier 0 CI blocks at `--severity=error`; run the default severity locally), `shfmt -d`, and PSScriptAnalyzer — see [PowerShell Linting](#powershell-linting). `shfmt` reads the repo-root `.editorconfig`, which records the tab indentation `shfmt` already defaults to, so editors follow the same convention the local run and CI's advisory step check.
- Markdown lint: `markdownlint-cli2 "**/*.md"`, configured by `.markdownlint.jsonc` (rules) and `.markdownlint-cli2.jsonc` (ignores). No workflow lints this repo's Markdown, so its findings are advisory; Tier 1's smoke test invokes `markdownlint-cli2` only to probe the baked binary's pinned version. The tree is not clean under it yet (81 findings across 26 files), so compare against the base branch rather than expecting a zero exit. See README "House Lint and Format Configuration".
...
| Bundled Playwright rationale (why browsers are not baked, why system chromium cannot substitute, persistent browsers cache) | [docs/architecture.md](docs/architecture.md) → "Bundled Playwright" |
