...
- **Shared image cache:** a single global, read-only `agent-podman-imagestore` volume (layered under every per-container graphroot via Podman's `additionalimagestores`) pre-seeds a small curated set of common dev images — `postgres`, `redis`, `mariadb`, `adminer` — so they resolve instantly without a per-container pull. The writable graphroot stays per-container; only the read path is shared. Override the curated set with `POWBOX_IMAGE_STORE_IMAGES`, or refresh it with `reseed-images`.
