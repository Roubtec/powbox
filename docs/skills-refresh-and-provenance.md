...
A *sidecar* marker is two independent renames, and a digest-carrying sidecar is where that bites: two containers publishing concurrently can interleave them and leave one's asset under the other's `sha256=`, after which the digest never matches again and the asset reads as customized until it is deleted. A sidecar carrying no digest survives the same interleaving — `seed_workflow`'s refresh turns on the marker's *presence*, so whichever refresher next runs republishes both (nothing is epoch-gated here: `seed_workflows … refresh` republishes whenever it is *invoked*, and on this tree no startup path invokes it at all — the republication a stale copy actually gets comes from the user-run `agent-update-skills` worker, not from a rebuild) — which is why the statusline is the only consumer here for which the pair has to be serialized at all.
