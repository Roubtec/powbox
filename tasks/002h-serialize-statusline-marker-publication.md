...
- Two containers from different newer image epochs starting concurrently against one `claude-config` volume can no longer leave the statusline paired with the wrong marker, for as long as both run the locked hook and neither publishes on an expired wait — always true under the skip-the-refresh fallback, and the standing cost of the proceed-unlocked one.
