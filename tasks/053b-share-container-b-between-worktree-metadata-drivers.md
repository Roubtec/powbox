...
Task 053a extracted **Container A**'s inner script into one file both drivers embed (`scripts/smoke-test-worktree-metadata-container-a.bash`), because a ~150-line Bash payload duplicated across `scripts/smoke-test-worktree-metadata.sh` and `scripts/smoke-test-worktree-metadata.ps1` is exactly how the coverage gap that created task 053a opened in the first place.
...
Unlike 053a, the positive half is now automated: Tier 1 runs **both** drivers' Stage 6 (`.github/workflows/native-linux-build.yml` gained a dedicated PowerShell step in 053a's PR), so a payload that is loaded-then-discarded, or that breaks inside the image, turns Tier 1 red rather than needing a maintainer's desktop.
...
- The old inline literal is **deleted** from each driver — not left in place — and reachability is proven directly, not inferred from the artifact being correct.
