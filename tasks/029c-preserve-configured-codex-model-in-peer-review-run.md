...
- Normalize the already-parsed final provider message into a new file inside the final attempt's returned `artifactDir`, then chmod and verify exact mode 0600 before constructing a `passed` or `issues` terminal result; fail the helper operationally if enforcement cannot be proved. Retain earlier attempts as sibling directories under the existing session parent, and do not make the stable payload path an alias for undocumented provider-native files.
...
- Exercise review-file mode enforcement for successful 0600 creation and for chmod failure, unreadable mode, and a silently ignored chmod; each negative case must fail operationally before any `passed` or `issues` JSON is emitted.
