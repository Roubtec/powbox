...
2. **Per-worktree cache location: `.worktrees/.golangci-cache/<container>/<slug>/`** beside the pnpm store (not inside the worktree — that would dirty `git status` in repos that don't ignore it and block `wt-remove`'s clean check). The extra `<container>` level mirrors the `.worktrees/<container>/<slug>` worktree layout, so the cache path is derived from the worktree path by a pure string mapping. `wt-remove` learns to delete the matching cache dir so nothing accumulates.
