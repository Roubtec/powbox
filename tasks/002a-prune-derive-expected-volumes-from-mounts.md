...
Generic follow-up from the PR #59 review (per-agent volume isolation). Was deferred because the current prune behavior is **fail-safe** (it over-protects, never over-deletes) and the fix expands `prune-volumes.{sh,ps1}` in both languages plus re-validation, while PR #59 is defendable as-is. **Now promoted to `tasks/` — approach decided below.**
...
**Chosen: approach A — derive the expected (protected) set from real `docker inspect` mounts.** Plus an explicit simplification: **do not** add any special-case logic to protect legacy / differently-named volumes (`agent-nm-<project>` etc.). Mount-derivation already protects such a volume *for free* whenever an existing container actually mounts it (running or stopped — the mount config persists), with no failed-removal noise; once **no existing container mounts it**, it is treated as an orphan and offered for pruning, which is exactly the "no longer being created" case we want reclaimed. The interactive confirm-delete prompt plus Docker's own "volume in use" refusal are the safety boundary, so no belt-and-suspenders legacy protection is needed.

**Considered & declined:** approach **B** (creation-time label) — more moving parts, and it only helps containers created after it ships; worth revisiting only once powbox runs across many people/machines.

## Suggested approach (chosen: A)
...
- A legacy / differently-named volume that **no existing container mounts** is listed as an orphan (no special-casing of old name structures — the confirm-delete prompt + Docker's in-use check are the backstop).
