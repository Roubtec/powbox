...
$script = @(
  ...
  '# fixed). PRIMARY: wired .Config.Healthcheck.Test -- CMD-SHELL "/bin/sh -c ..." is the'
  '# mistranslation (XFAIL, stay GREEN); a preserved CMD exec form SELF-CLEARS to a loud'
  '# NOTE (true provider-fixed signal). CAUSE: the health Log Output is empty in podman'
  '# 5.x, so run the wrap /bin/sh via podman exec and capture crun reason -- /bin/sh is'
  '# ABSENT (/pause the binary IS present), so the wrap is the cause. SKIP above on pull fail.'
  'if [ "$distroless_ok" = true ]; then'
  ...
  '  dl_wrapped=false'
  '  case "$dl_kind" in CMD-SHELL) case "$dl_test" in */bin/sh*) dl_wrapped=true ;; esac ;; esac'
  '  dl_shellmissing=false'
  '  if dl_shellerr=$(podman exec "$dl_cid" /bin/sh -c ":" 2>&1); then :; else case "$dl_shellerr" in */bin/sh*) dl_shellmissing=true ;; esac; fi'
  ...
  '    echo "KNOWN-XFAIL (distroless): podman-compose 1.3 wrapped the shell-less service exec-form check into ${dl_test}, reintroducing /bin/sh -- which is ABSENT from this image (proven: podman exec /bin/sh -> ${dl_shellerr}). The /pause binary IS present (image entrypoint), so the shell wrap -- not an absent check binary and not a never-exiting one -- is why the check fails (driven state: ${dl_health}). This reproduces the Kalm2 distroless SPIRE break and is the EXPECTED green state here. See docs/rootless-podman.md Compose health-check behavior."'
