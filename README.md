...
This native-Linux bind-mount uid expectation — a `root`-owned (or mixed-ownership) mount must end up writable by `node` (uid 1000) — is regression-guarded by the dir-mount stage of the smoke test (`scripts/smoke-test-dirmount.{sh,ps1}`, Stage 5 of `commands/smoke-test.*`). It builds two git fixtures — a genuinely `root`-owned tree and a mixed-ownership one (a `node`-owned root hiding a nested `root`-owned tracked file and a `.git/objects/<xx>` shard) — and asserts `node` can edit and `git commit` each after the `fix-workspace-perms.sh` re-own. To keep that assertion isolated the stage overrides the image entrypoint and invokes the helper directly, so it guards the baked helper plus its sudoers wiring rather than `entrypoint-core.sh`'s own write-probe and nested-uid-0 scan — booting the real entrypoint end to end is the follow-up tracked in [`tasks/005a-dirmount-smoke-exercise-real-entrypoint.md`](tasks/005a-dirmount-smoke-exercise-real-entrypoint.md), with exercising the entrypoint's nested-uid-0 scan specifically tracked in [`tasks/007a-smoke-exercise-entrypoint-nested-uid0-scan.md`](tasks/007a-smoke-exercise-entrypoint-nested-uid0-scan.md). Because the bug is native-Linux-only (Windows/WSL/macOS bind mounts mask it) and a `root`-owned fixture needs root, the stage self-skips off native-Linux and on hosts without root/passwordless sudo, and runs for real on the CI runner.
