...
Because a statusline cannot observe that approval, Claude's shows `env auth` in place of the account name whenever one of the three variables is **non-empty** — note that Compose always *sets* `ANTHROPIC_API_KEY`, usually to the empty string, which does not trigger the label. Read it as a signal that the cached OAuth name may not be the identity in use, not as a claim about which credential won. That label reaches an existing container only after you delete `~/.claude/statusline-command.sh`, which is seeded no-clobber.
